...
function request(server: http.Server, path: string, method = 'GET'): Promise<{ statusCode: number; body: string }> {
...
describe('VerdaccioMiddlewarePlugin', () => {
  ...
  it('does not count HEAD tarball probes', async () => {
    const app = express();
    const plugin = new VerdaccioMiddlewarePlugin(
      { redis: {} } as any,
      { logger: { error: jest.fn(), warn: jest.fn() } } as any,
    ...
    const server = app.listen(0);
    try {
      const response = await request(server, url, 'HEAD');
      expect(response.statusCode).toBe(204);
      expect(downstreamUrl).toBe(url);
      expect(storage.getPackage).not.toHaveBeenCalled();
    } finally {
      await new Promise<void>((resolve, reject) => {
        server.close((err?: Error) => {
          if (err) {
